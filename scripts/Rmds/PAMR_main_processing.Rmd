...
```{r, fig.height=8, fig.width=4, dev='svg'}
pamrcv
pamr.plotcv(pamrcv)
```
...
Saving also sd estimated for each gene by the algorithm (pooling them):

```{r}
write.csv(as.data.frame(pamr$sd), here::here("output/pamr_estimated_sd.csv"), row.names = TRUE)
```
...
```{r, fig.height=7.5, fig.width=10, dev='svg'}
...
```{r, fig.width=8, fig.height=3, dev='svg'}
...
cfm$table
silplot(vcrtestTo, main=paste0("(testing) Silplot at threshold: ",To))
```

```{r, dev='svg'}
cfm=caret::confusionMatrix(factor(vcrtestTh$pred),factor(vcrtestTh$ynew))
cfm$table
silplot(vcrtestTh, main=paste0("(testing) Silplot at threshold: ",Th))
```
